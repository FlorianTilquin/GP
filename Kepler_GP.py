...
def Kepler_GP(Kid , Beg , End , Landscale , sigma, Z , p = 7, alpha = 1, plot = 0, plottitle = 0):  
    ...
    H = eye(1, p+1)
    I = eye(p+1)
    phi = sqrt(2)/Landscale

    # Karma Filter Matrices
    [F , L , tilde , q , p0] = materngp2ss(phi , sigma , p)
    [A , Q] = lti_disc(F , L , q , 0.0204) #dt = 0.0204 for Kepler data
